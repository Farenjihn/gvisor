...
// Valid implements kernfs.inodeLookup.Valid. This inode remains valid as long
...
func (fs *filesystem) newTaskOwnedDir(task *kernel.Task, ino uint64, perm linux.FileMode, children map[string]kernfs.Inode) kernfs.Inode {
	// Note: credentials are overridden by taskOwnedInode.
	dir := kernfs.NewStaticDir(task.Credentials(), linux.UNNAMED_MAJOR, fs.devMinor, ino, perm, children, kernfs.GenericDirectoryFDOptions{SeekEnd: kernfs.SeekEndZero})
