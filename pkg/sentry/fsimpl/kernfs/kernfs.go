...
// Reference Model:
//
// Kernfs dentries represents named pointers to inodes and nodes on the
// filesystem dentry tree. Kernfs is solely reponsible for maintaining and
// modifying its dentry tree; inodes can not access the tree. Dentries and
// inodes have independent lifetimes and reference counts. A child dentry
// unconditionally holds a reference on its parent directory's dentry. A dentry
// also holds a reference on the inode it points to (although that might not be
// the only reference on the inode). Due to this inodes can outlive the
// dentries that point to them. Multiple dentries can point to the same
...
type Inode interface {
	...
	// Method for inodes that represent directories and provide access to its
	// children.
	inodeLookup
...
type inodeLookup interface {
	...
	// layer) if no other ref is picked on that dentry.
