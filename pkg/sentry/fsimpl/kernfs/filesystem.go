...
func (fs *Filesystem) revalidateChildLocked(ctx context.Context, vfsObj *vfs.VirtualFilesystem, parent *Dentry, name string, child *Dentry) (*Dentry, error) {
	if child != nil {
		...
		if !child.inode.Valid(ctx) {
			...
			fs.deferDecRef(parent) // IncRef from Dentry.insertChild.
			if child.inode.Keep() {
	...
	if child == nil {
		...
		childI, err := parent.inode.Lookup(ctx, name)
		if err != nil {
			return nil, err
		}
		var c Dentry
		c.Init(fs, childI) // childI's ref is transferred to c.
		parent.insertChildLocked(name, &c)
		child = &c

		// Drop the ref on c. This will cause the dentry to get pruned from the
		// dentry tree by the end of current filesystem operation (before
		// returning to the VFS layer) if another ref is not picked on this
		// dentry.
		if !childI.Keep() {
			fs.deferDecRef(&c)
		}
...
func (fs *Filesystem) OpenAt(ctx context.Context, rp *vfs.ResolvingPath, opts vfs.OpenOptions) (*vfs.FileDescription, error) {
	...
	// Since we defer unlock() AFTER this, fs.mu is garaunteed to be unlocked
...
func (fs *Filesystem) RenameAt(ctx context.Context, rp *vfs.ResolvingPath, oldParentVD vfs.VirtualDentry, oldName string, opts vfs.RenameOptions) error {
	...
	if replaced != nil {
		...
		fs.deferDecRef(dstDir) // IncRef from Dentry.insertChild.
...
func (fs *Filesystem) RmdirAt(ctx context.Context, rp *vfs.ResolvingPath) error {
	...
	fs.deferDecRef(parentDentry) // IncRef from Dentry.insertChild.
	fs.deferDecRef(d)
...
func (fs *Filesystem) deferDecRefVD(ctx context.Context, vd vfs.VirtualDentry) {
	if d, ok := vd.Dentry().Impl().(*Dentry); ok {
		// The following is equivalent to vd.DecRef(ctx). This
		// is needed because we can not DecRef a kernfs dentry while
		// holding any locks.
		vd.Mount().DecRef(ctx)
		fs.deferDecRef(d)
	} else {
